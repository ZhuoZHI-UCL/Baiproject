...
dataset=pd.read_excel('/scratch/uceezzz/Project/Baiproject/Dataset/0814_GCO.xlsx')#这个是150条的
test_dataset = pd.read_excel('/scratch/uceezzz/Project/Baiproject/Dataset/prediction_source.xlsx')#这个是50条的

feature=dataset.iloc[:,1:-1] #总共是27个特征哈  注意，这个地方是取不到-2的，只能取到-3
feature = feature.iloc[:,[0, 1, 2, 5, 6, 7, 10, 11, 12, 15, 16, 17, 20, 21, 23, 24, 25, 26]]
label=dataset.iloc[:,-1].values

test_feature = test_dataset.iloc[:,1:]
test_feature = test_feature.iloc[:,[0, 1, 2, 5, 6, 7, 10, 11, 12, 15, 16, 17, 20, 21, 23, 24, 25, 26]]
...
prediction_dataset['GCO'] = test_pred_mean
prediction_dataset.to_excel('/scratch/uceezzz/Project/Baiproject/Dataset/prediction.xlsx')
print('All done!')

'''    
f1 mean:  0.6847619047619047
f1 std:  0.22119854924013638
'''     
