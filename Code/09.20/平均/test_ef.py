...
dataset=pd.read_excel('/scratch/uceezzz/Project/Baiproject/Dataset/0920/训练-平均.xlsx')#这个是150条的
test_dataset = pd.read_excel('/scratch/uceezzz/Project/Baiproject/Dataset/0920/测试-平均.xlsx')#这个是50条的

feature=dataset.iloc[:,1:-3] #总共是27个特征哈  注意，这个地方是取不到-2的，只能取到-3
feature = feature.iloc[:,[0, 2, 6]]
label=dataset.iloc[:,-2].values

test_feature = test_dataset.iloc[:,1:]
test_feature = test_feature.iloc[:,[0, 2, 6]]
...
prediction_dataset = pd.read_excel('/scratch/uceezzz/Project/Baiproject/Dataset/0920/测试-平均-最终.xlsx')#这个是50条的
prediction_dataset['Ef'] = test_pred_mean
prediction_dataset.to_excel('/scratch/uceezzz/Project/Baiproject/Dataset/0920/测试-平均-最终.xlsx')
print('All done!')

'''      
f1 mean:  0.7047619047619047
f1 std:   0.20769766969194633
'''     
